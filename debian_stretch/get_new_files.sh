...
for LVAR_DEBIAN_DIST in amd64 arm32v7 arm64v8 i386; do
	...
	TMP_NFN="rootfs-debian_${LVAR_DEBIAN_RELEASE}_${LVAR_DEBIAN_VERSION}-${LVAR_DEBIAN_DIST}.tar.xz"
	mv "tmpdown/rootfs.tar.xz" "$TMP_NFN" || break
	md5sum_poly "$TMP_NFN" > "$TMP_NFN.md5" || break
done
